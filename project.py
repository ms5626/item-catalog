...
#Authenticate user using Facebook API
...
#Disconnect user logged in via Facebook
...
def fbdisconnect():
    ...
    if result['status'] == '200':
        ...
        response.headers['Content-Type'] = 'application/json'
        flash("You could not be logged out.")
        return response
...
#Create new user and store in the User table
...
#Disconnect user logged in via google.
...
# JSON APIs to view Restaurant and Menu Information
...
# JSON APIs to view Specific Menu Information
@app.route('/restaurant/<int:restaurant_id>/menu/<int:menu_id>/JSON')
def menuItemJSON(restaurant_id, menu_id):
    Menu_Item = session.query(MenuItem).filter_by(id=menu_id).one()
    return jsonify(Menu_Item=Menu_Item.serialize)

# JSON APIs to view all Restaurants Information
